...
int main(int argc, char const *argv[])
{
    ...
    // 构建 SQL 更新语句
    char sql_syntax[200];
    snprintf(sql_syntax, sizeof(sql_syntax), "UPDATE user SET uname = '%s', pwd = '%s' WHERE ic = '%s'", username, password, icCard);
    ...
    // 执行删除操作
    db_update(sql, sql_syntax);

    // 关闭数据库连接
    close_db_connection(sql);

    // 构建并输出 JSON 响应
    printf("{\"success\": true, \"message\": \"更新成功\"}");
